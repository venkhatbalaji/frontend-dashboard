...
import { getViolatorsByGender } from "@/services/activeResidenceService";
...
function ViolatorsByGender({ isPreviewOpen, pageRef, filters, isRtl }) {
  const intl = useIntl();
    ...
    <DashboardCard
      ...
      title={
        <Row align="middle" gutter={4}>
          <Col flex="none">
            {intl?.formatMessage({ id: "Violators by Gender" })}
          </Col>
          <Col flex="none">
            <Tooltip
              title={intl?.formatMessage({ id: "active_residence_violators_gender_tooltip" })}
            >
              <span>
...
export default ViolatorsByGender;