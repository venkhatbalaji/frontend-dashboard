...
export default function NationalityStatistics({ emiratesConfigValue }) {
  ...
  const handleExport = () => {
    const title = activeCountryName
      ? `${translation[language]?.["Nationality Statistics"] || "Nationality Statistics"} - ${activeCountryName}.pdf`
  ...
  return (
    <>
      <Row
        ...
        align="middle"
        wrap={false}
        style={{ marginBottom: "8px", direction: "ltr" }}
      >
        {isRtl ? (
          <>
            <Col flex="none">
      ...
      {!activeCountry ? (
        ...
        <NationalityStatisticsCountryPanel
          key={activeCountry}
