...
export default function NationalityStatisticsCountryPanel({ country, countryName, emiratesConfigValue, geoJsonObj, isCreatingPdf }) {
  const [localeStore] = useContext(LocaleContext);
  const language = localeStore?.projectTranslation || "en";

  const referenceDate = useMemo(() => {
    return country === "PAK" ? "2026-03-28" : "2026-02-28";
  }, [country]);
  ...
  React.useEffect(() => {
    ...
    executeCancellations({
      filter: { nationality_code: country, snapshot_date: today },
    });
    executeSnapshot({
      filter: {
  ...
  return (
    <Page
      title={
        countryName
          ? `${translation[language]?.["Updates on the Nationality in the UAE"] || "Updates on the Nationality in the UAE"} - ${countryName}`
                ...
                <Row align={"middle"} justify={"center"} isFullHeight>
                  <Col flex={"none"}>
                    <CountryFlagDisplay
                      ...
                      countryName={
                        countryName
                          ? `${translation[language]?.["Republic of"] || "Republic of"} ${countryName}`
                          : ""
                      }
                      language={language}
                    />
...
NationalityStatisticsCountryPanel.propTypes = {
