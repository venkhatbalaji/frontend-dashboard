...
import { Row, Col, Spin, Empty, Text } from "re-usable-design-components";
...
// `country` is undefined for the pinned "All Nationalities" tab, which also
// shows the extra "Top Risk By Nationality" section that a specific
// country's report doesn't.
export default function RiskRegisterStatisticsCountryPanel({ country, countryName, isCreatingPdf }) {
  ...
  // This panel is only ever mounted for one specific, already-chosen tab
  // (the container remounts it via `key={country ?? "__ALL__"}` on tab switch),
  // so the fetch effect only needs to react to a language switch.
  useEffect(() => {
    const apiFilters = { nationality: country };
  ...
  return (
    <Page
      title={
        countryName
          ? `${translation?.[language]?.["Risk Register Statistics"] || "Risk Register Statistics"} - ${countryName}`
...
RiskRegisterStatisticsCountryPanel.propTypes = {
