...
import { Row, Col, Empty, Text, Spin } from "re-usable-design-components";
import moment from "moment";
import { LocaleContext } from "@/globalContext/locale/localeProvider";
import { formatNumber, checkIsPdfSectionEmpty } from "@/utils/helper";
...
export default function ExpatsStatisticsCountryPanel({ country, countryName, emiratesConfigValue, geoJsonObj, isCreatingPdf }) {
  ...
  // This panel is only ever mounted for one specific, already-chosen country
  // (the container remounts it via `key={country}` when the active tab changes),
  // so the fetch effect only needs to react to a language switch.
  React.useEffect(() => {
    const end_date = moment().format("YYYY-MM-DD");
    const start_date = moment().subtract(365, 'days').format("YYYY-MM-DD");

    executeTotalExpats({ filter: { nationality_code: country } });
    executeIssuedResidenceVisa({ filter: { nationality: country, type: 'residence' } });
    executeIssuedVisas({ filter: { nationality: country, type: 'visa' } });
    executeExpatsByGender({ filter: { nationality_code: country } });
    executeExpatsByEmirate({ filter: { nationality: country } });
    executeExpatsByAgeRange({ filter: { nationality_code: country } });
    executeViolatorsSummary({ filter: { nationality: country, language: language } });
    executeViolatorsByGender({ filter: { nationality: country } });
    executeRiskTypesByRegister({ filter: { nationality: country, language, start_date, end_date } });
  ...
  const { isLoading: isAnySectionLoading, allSectionsEmpty } = useMemo(() => {
  ...
  return (
    <Page title={
      countryName
        ? `${translation[language]?.["Expats in the UAE"] || "Expats in the UAE"} - ${countryName}`
            ...
            <Col>
              <Summary
                ...
                CountryFlag={(() => {
                  ...
                  const iso2Code = getISO2Code(country);
...
ExpatsStatisticsCountryPanel.propTypes = {
  country: PropTypes.string.isRequired,
  countryName: PropTypes.string,
  emiratesConfigValue: PropTypes.object,
  geoJsonObj: PropTypes.object,
