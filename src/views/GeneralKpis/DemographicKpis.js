...
import {
  ...
  Text, Slider, RadioGroup, Card, Modal, DateRangePicker, AntIcons
...
import PlaneGlobe from "@/svgr/PlaneGlobe";
import MotherDaughter from "@/svgr/MotherDaughter";
import HandLink from "@/svgr/HandsLink";
...
import useDependencyRatio from "./hooks/useDependencyRatio";
import useFertilityRate from "./hooks/useFertilityRate";
import useMigrationRate from "./hooks/useMigrationRate";
...
const PopulationGrowth = withHook({ Component: LineBarChart, useHook: usePopulationGrowth })
const DependencyRatio = withHook({ Component: LineBarChart, useHook: useDependencyRatio })
const FertilityRate = withHook({ Component: LineBarChart, useHook: useFertilityRate })
const MigrationRate = withHook({ Component: LineBarChart, useHook: useMigrationRate })
const AgeDistribution = withHook({ Component: LineBarChart, useHook: useAgeDistribution })
...
function DemographicKpis({ nationalitiesConfigValueObj, residencyTypeValues, emiratesConfigValue }) {
  ...
  const intl = useIntl();
  const titleText = intl.formatMessage({ id: "Demographic KPIs" });
  ...
  const fertilityRateEle = (
    <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
      <FertilityRate
        icon={<MotherDaughter />}
        isRtl={isRtl}
        filters={filters}
        tooltipKey={intl?.formatMessage({ id: "Fertility_Rate_Tooltip" })}
        title={intl?.formatMessage({ id: "Total Fertility Rate" })}
        pageRef={pageRef}
  ...
  const populationAgeEle = (
    <Col span={patchedGetResponsive({ default: 16, tablet: 24, midTablet: 24 })}>
      <AgeDistribution
        icon={<AgeRange />}
        isRtl={isRtl}
        isBarChartOnly
        filters={filters}
        title={intl?.formatMessage({ id: "Population Age Distribution" })}
        tooltipKey={intl?.formatMessage({ id: "Population_Age_Distribution_Tooltip" })}
        pageRef={pageRef}
      />
    </Col>
  )
    ...
    <Row gutter={[patchedGetResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <PopulationGrowth
          ...
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Population Growth Rate" })}
          tooltipKey={intl?.formatMessage({ id: "Population_Growth_Rate_Tooltip" })}
          pageRef={pageRef}
          isPreviewOpen={isPreviewOpen}
        />
      </Col>
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <DependencyRatio
          ...
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Demographic Dependency Ratio" })}
          tooltipKey={intl?.formatMessage({ id: "Demographic_Dependency_Ratio_Tooltip" })}
          pageRef={pageRef}
          isPreviewOpen={isPreviewOpen}
        />
      </Col>
    ...
    <Row gutter={[patchedGetResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      ...
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <MigrationRate
          ...
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Net Migration Rate" })}
          tooltipKey={intl?.formatMessage({ id: "Net_Migration_Rate_Tooltip" })}
          pageRef={pageRef}
          isPreviewOpen={isPreviewOpen}
        />
      </Col>
  ...
  const titleEle = (
    <Row  gutter={8} align="middle">
      <Col flex="none" style={{ color: "var(--colorText)" }}>
        <ProjectOutlined style={{ fontSize: getResponsive({ default: "32px", tablet: "24px" }) }} />
      </Col>
      <Col flex="none">
            ...
            <span style={{ position: "relative" }}>
              <span>
                <Title level={getResponsive({ default: 4, mobile: 5 })}>
                  ...
                  <Tooltip
                    title={intl?.formatMessage({ id: "Demographic_KPIs_Title_Tooltip" })}
                    placement="bottom"
                  >
