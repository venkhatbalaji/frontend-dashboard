...
import { getCrimeOccupationsStatistics, getCrimeOccupations } from "@/services/generalIndicatorsService";
...
function InputWrap({ onSearch, onChange, typed, setTyped, filter, searchText, appliedSelectedOptions, setAppliedSelectedOptions, searchOptions, selectedOptions, setSelectedOptions }) {
  ...
  } = useAsync({ asyncFunction: getCrimeOccupations });
  ...
  useEffect(() => {
    if (status === "success") {
      setOptions(_.cloneDeep(departmentValue?.data))
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const optionValues = useMemo(() => {
    return options?.sort((a, b) => {
      return a?.[isRtl ? "occupation_ar" : "occupation_en"]?.localeCompare(b?.[isRtl ? "occupation_ar" : "occupation_en"])
    }).map((v) => ({
      value: v?.occupation_code,
      label: isRtl ? v?.occupation_ar : v?.occupation_en,
    }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [options])
...
function useCrimeOccupations({ filters }) {
  const [localeStore] = useContext(LocaleContext);
  const isRtl = checkRtl(localeStore)
  ...
  const {
    ...
    asyncFunction: getCrimeOccupationsStatistics
  });
  ...
  useEffect(() => {
    ...
    execute({
      filters: {
        ...filters,
        occupations: appliedSelectedOptions?.map((v) => v?.value),
        ...sortBy,
  ...
  const {
    execute: loadMoreOccupations,
  } = useAsync({ asyncFunction: getCrimeOccupationsStatistics });

  const isLoading = status === "idle" || status === "pending";
  // const isEmpty = !isLoading && !value?.data?.occupations?.length

  const getResponsive = useResponsive();
  const intl = useIntl();

  const formattedData = useMemo(() => {
    return (value?.data?.occupations || []);
  ...
  async function loadMoreValues() {
    if (filteredData?.length < value?.pagination?.total) {
      ...
      const res = await loadMoreOccupations({
        filters: {
          ...filters,
          ...sortBy,
      ...
      if (res?.data?.occupations?.length) {
        setState((v) => ({
          ...v,
          value: {
            ...v?.value,
            data: {
              occupations: [...(v?.value?.data?.occupations || []), ...(res?.data?.occupations || [])]
            }
          }
        }))
      }
  ...
  function handleScroll(e) {
    const ele = document.querySelector("#CrimeOccupationsTable .ant-table-body");
    if (!ele) return;
  ...
  return {
    ...
    status,
    columns,
    tableId: "CrimeOccupationsTable",
    onChange: handleTableChange
  };
}

export default useCrimeOccupations; 