...
import { getCrimeEconomicSectorStatistics } from "@/services/generalIndicatorsService";
...
function useCrimeEconomicSector({ filters }) {
  ...
  const { isLoading, value, isEmpty, status, chartName } = useBaseHook({
    asyncFunction: getCrimeEconomicSectorStatistics,
    filters,
    dataPath: 'economicSectors',
  });
  const getResponsive = useResponsive();
  const intl = useIntl();
  const formattedData = useMemo(() => {
    return (value?.data?.economicSectors || []);
  ...
  const columns = [{
    title: intl?.formatMessage({ id: "Economic Sector" }),
...
export default useCrimeEconomicSector; 