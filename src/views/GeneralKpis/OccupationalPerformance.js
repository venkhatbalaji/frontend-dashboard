...
import useEmploymentRate from "./occupationalPerformanceHooks/useEmploymentRate";
import useUnemploymentRate from "./occupationalPerformanceHooks/useUnemploymentRate";
import useSkilledWorker from "./occupationalPerformanceHooks/useSkilledWorker";
import useExpatriateWorker from "./occupationalPerformanceHooks/useExpatriateWorker";
import useWomenParticipation from "./occupationalPerformanceHooks/useWomenParticipation";
import useEconomicSector from "./occupationalPerformanceHooks/useEconomicSector";
import HandMessage from "@/svgr/HandMessage";
import MenStars from "@/svgr/MenStars";
import MenForm from "@/svgr/MenForm";
import GlobleCircleHome from "@/svgr/GlobleCircleHome";
import GirlLaptop from "@/svgr/GirlLaptop";
import SuitcaseWithCross from "@/svgr/SuitcaseWithCross";
import MainContentWrapDashboards from "@/components/MainContentWrapDashboards";


dayjs.extend(quarterOfYear);

const { Suitcase } = PhosphorIcons;


const EmploymentRate = withHook({ Component: LineBarChart, useHook: useEmploymentRate })
const UnemploymentRate = withHook({ Component: LineBarChart, useHook: useUnemploymentRate })
const SkilledWorker = withHook({ Component: LineBarChart, useHook: useSkilledWorker })
const ExpatriateWorker = withHook({ Component: LineBarChart, useHook: useExpatriateWorker })
const WomenParticipation = withHook({ Component: LineBarChart, useHook: useWomenParticipation })
const EconomicSector = withHook({ Component: LineBarChart, useHook: useEconomicSector })
...
function DemographicKpis({ nationalitiesConfigValueObj, residencyTypeValues, emiratesConfigValue }) {
  ...
  const intl = useIntl();
  const titleText = intl.formatMessage({ id: "Occupational Performance" });
  const {
    isCreatingPdf,
    printDocument,
    printDocumentCanvas,
    ...
    <Row gutter={[patchedGetResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <EmploymentRate
          ...
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Employment Rate" })}
          tooltipKey={intl?.formatMessage({ id: "Employment_Rate_Tooltip" })}
          pageRef={pageRef}
      ...
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <UnemploymentRate
          icon={<SuitcaseWithCross />}
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Unemployment Rate" })}
          tooltipKey={intl?.formatMessage({ id: "Unemployment_Rate_Tooltip" })}
          pageRef={pageRef}
          isPreviewOpen={isPreviewOpen}
        />
      </Col>
    ...
    <Row gutter={[patchedGetResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      ...
      <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
        <ExpatriateWorker
          ...
          isRtl={isRtl}
          filters={filters}
          title={intl?.formatMessage({ id: "Expatriate Worker Ratio" })}
          tooltipKey={intl?.formatMessage({ id: "Expatriate_Worker_Ratio_Tooltip" })}
          pageRef={pageRef}
          isPreviewOpen={isPreviewOpen}
        />
      </Col>
  ...
  const skilledWorkerEle = (
    <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
      <SkilledWorker
        ...
        isRtl={isRtl}
        filters={filters}
        title={intl?.formatMessage({ id: "Skilled Worker Ratio" })}
        tooltipKey={intl?.formatMessage({ id: "Skilled_Worker_Ratio_Tooltip" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const womenParticipationEle = (
    <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
      <WomenParticipation
        ...
        isRtl={isRtl}
        filters={filters}
        title={intl?.formatMessage({ id: "Women's Workforce Participation" })}
        tooltipKey={intl?.formatMessage({ id: "Women_Workforce_Participation_Tooltip" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const economicSectorEle = (
    <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
      <EconomicSector
        ...
        isRtl={isRtl}
        filters={filters}
        tooltipKey={intl?.formatMessage({ id: "Career_Transfer_Rate_Tooltip" })}
        title={intl?.formatMessage({ id: "Career Transfer Rate" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const titleEle = (
    <Row gutter={8} align="middle">
      <Col flex="none" style={{ color: "var(--colorText)" }}>
        <Suitcase size={getResponsive({ default: "32px", tablet: "24px" })} />
      </Col>
      <Col flex="none">
        ...
        >
          ...
          >
            <Title level={getResponsive({ default: 4, mobile: 5 })}>
              ...
              <Tooltip
                title={intl?.formatMessage({ id: "Occupational_Performance_Title_Tooltip" })}
                placement="bottom"
              >
                <span style={{ marginInline: "4px", marginBottom: "2px" }}>
