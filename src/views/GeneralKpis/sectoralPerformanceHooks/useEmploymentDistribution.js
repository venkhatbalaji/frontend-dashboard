...
import { getEmploymentDistributionStatistics } from "@/services/generalIndicatorsService";
...
function useEmploymentDistribution({ filters }) {
  ...
  const { isLoading, value, isEmpty, status, chartName } = useBaseHook({
    asyncFunction: getEmploymentDistributionStatistics,
    filters,
    dataPath: 'sectors',
  });
  ...
  {
    title: intl?.formatMessage({ id: "Total Employees" }),
