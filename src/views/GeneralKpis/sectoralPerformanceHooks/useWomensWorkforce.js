...
import { getWomensWorkforceStatistics } from "@/services/generalIndicatorsService";
...
function useEmploymentDistribution({ filters }) {
  ...
  const { isLoading, value, isEmpty, status, chartName } = useBaseHook({
    asyncFunction: getWomensWorkforceStatistics,
    filters,
    dataPath: 'womensWorkforceParticipation',
  });
  const getResponsive = useResponsive();
  const intl = useIntl();

  const formattedData = useMemo(() => {
    return (value?.data?.womensWorkforceParticipation || []);
  }, [value?.data?.womensWorkforceParticipation]);
  ...
  {
    title: intl?.formatMessage({ id: "Total Participation" }),
    width: getResponsive({ default: "50%" }),
    sorter: {
      compare: (a, b) => a?.women_workers - b?.women_workers,
    },
    render: (v) => {
      ...
      return (
        <Row align="middle" gutter={getResponsive({ default: [12], tablet: [0, 0], midTablet: [12], mobile: [0, 0] })}>
          <Col flex={getResponsive({ default: "0 0 95px", mobile: "0 0 95px" })}>
            <Text
              ellipsis={{
                tooltip: v?.women_workers ? formatNumber(v?.women_workers) : '-'
              }}
            >
              {v?.women_workers ? formatNumber(v?.women_workers) : '-'}
            </Text>
          </Col>
