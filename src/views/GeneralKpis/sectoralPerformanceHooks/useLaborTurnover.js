...
import { getLaborTurnoverStatistics } from "@/services/generalIndicatorsService";
...
function useEmploymentDistribution({ filters }) {
  ...
  const { isLoading, value, isEmpty, status, chartName } = useBaseHook({
    asyncFunction: getLaborTurnoverStatistics,
    filters,
    dataPath: 'labourTurnover',
  });
  const getResponsive = useResponsive();
  const intl = useIntl();

  const formattedData = useMemo(() => {
    return (value?.data?.labourTurnover || []);
  }, [value?.data?.labourTurnover]);
  ...
  const columns = [{
    ...
    width: getResponsive({ default: isRtl ? "35%" : "50%", mobile: "155px" }),
    ellipsis: true,
    sorter: {
  ...
  {
    title: intl?.formatMessage({ id: "Turnover Rate" }),
    width: getResponsive({ default: isRtl ? "65%" : "50%" }),
    sorter: {
      compare: (a, b) => a?.total_leavers - b?.total_leavers,
    },
    render: (v) => {
      const percent = v?.turnover_rate?.toFixed(2);
      ...
      return (
        <Row align="middle" gutter={getResponsive({ default: [12], tablet: [0, 0], midTablet: [12], mobile: [0, 0] })}>
          <Col flex={getResponsive({ default: "0 0 95px", mobile: "0 0 95px" })}>
            <Text
              ellipsis={{
                tooltip: v?.total_leavers ? formatNumber(v?.total_leavers) : '-'
              }}
            >
              {v?.total_leavers ? formatNumber(v?.total_leavers) : '-'}
            </Text>
          </Col>
