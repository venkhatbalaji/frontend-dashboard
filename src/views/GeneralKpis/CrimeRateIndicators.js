...
import useCrimeRate from "./crimeRateHooks/useCrimeRate";
import useCrimeExpatriateWorker from "./crimeRateHooks/useCrimeExpatriateWorker";
import useCrimeFrequency from "./crimeRateHooks/useCrimeFrequency";
import useCrimeType from "./crimeRateHooks/useCrimeType";
import useCrimeNationality from "./crimeRateHooks/useCrimeNationality";
import useCrimeEconomicSector from "./crimeRateHooks/useCrimeEconomicSector";
import useCrimeOccupations from "./crimeRateHooks/useCrimeOccupations";
...
import Barricade from "@/svgr/Barricade";
import CriminalHandcuff from "@/svgr/CriminalHandcuff.js";
import MainContentWrapDashboards from "@/components/MainContentWrapDashboards";


dayjs.extend(quarterOfYear);

const { Gavel, Crosshair, Briefcase } = PhosphorIcons;


const CrimeRate = withHook({ Component: LineBarChart, useHook: useCrimeRate })
const CrimeExpatriateWorker = withHook({ Component: LineBarChart, useHook: useCrimeExpatriateWorker })
const CrimeFrequency = withHook({ Component: LineBarChart, useHook: useCrimeFrequency })
const CrimeType = withHook({ Component: Table, useHook: useCrimeType })
const CrimeNationality = withHook({ Component: Table, useHook: useCrimeNationality })
const CrimeEconomicSector = withHook({ Component: Table, useHook: useCrimeEconomicSector })
const CrimeOccupations = withHook({ Component: Table, useHook: useCrimeOccupations })
...
function CrimeRateIndicators({ nationalitiesConfigValueObj, residencyTypeValues, emiratesConfigValue }) {
  ...
  const intl = useIntl();
  const titleText = intl.formatMessage({ id: "Crime Related Indicators" });
  const {
    isCreatingPdf,
    printDocument,
    printDocumentCanvas,
  ...
  const skilledWorkerEle = (
    <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
      <CrimeFrequency
        icon={<CriminalHandcuff />}
        isRtl={isRtl}
        filters={filters}
        title={intl?.formatMessage({ id: "Criminal Frequency Rate" })}
        tooltipKey={intl?.formatMessage({ id: "Criminal_Frequency_Rate_Tooltip" })}
        pageRef={pageRef}
  ...
  const crimeNationalityEle = (
    <Col span={patchedGetResponsive({ default: 12, tablet: 12, midTablet: 24 })}>
      <CrimeNationality
        icon={<WorldWithPopulation />}
        isRtl={isRtl}
        nationalitiesConfigValueObj={nationalitiesConfigValueObj}
        filters={filters}
        title={intl?.formatMessage({ id: "Crime by Country" })}
        tooltipKey={intl?.formatMessage({ id: "Crime_by_Nationality_Tooltip" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const economicSectorEle = (
    <Col span={patchedGetResponsive({ default: 12, tablet: 12, midTablet: 24 })}>
      <CrimeEconomicSector
        ...
        isRtl={isRtl}
        filters={filters}
        tooltipKey={intl?.formatMessage({ id: "Crime_Rate_by_Economic_Sector_Tooltip" })}
        title={intl?.formatMessage({ id: "Crime Rate by Economic Sector" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const crimeOccupationEle = (
    <Col span={patchedGetResponsive({ default: 12, tablet: 24, midTablet: 24 })}>
      <CrimeOccupations
        icon={<Briefcase weight="light" size={32} />}
        isRtl={isRtl}
        filters={filters}
        tooltipKey={intl?.formatMessage({ id: "Crime_by_Occupation_Tooltip" })}
        title={intl?.formatMessage({ id: "Crime by Occupation" })}
        pageRef={pageRef}
        isPreviewOpen={isPreviewOpen}
      />
    </Col>
  ...
  const scrollableElements = () => (
    ...
    <Col>
      <Row wrap={true} gutter={[patchedGetResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), getResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM })]}>
        <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
          <CrimeRate
            icon={<Crosshair weight="light" size={32} />}
            isRtl={isRtl}
            filters={filters}
            title={intl?.formatMessage({ id: "Crime Rate Per 100,000" })}
            tooltipKey={intl?.formatMessage({ id: "Crime_Rate_Per_Tooltip" })}
            pageRef={pageRef}
            isPreviewOpen={isPreviewOpen}
          />
        </Col>
        <Col span={patchedGetResponsive({ default: 8, tablet: 12, midTablet: 24 })}>
          <CrimeExpatriateWorker
            icon={<GlobleCircleHome />}
            isRtl={isRtl}
            filters={filters}
            title={intl?.formatMessage({ id: "Expatriate Worker Crime Rate" })}
            tooltipKey={intl?.formatMessage({ id: "Expatriate_Worker_Crime_Rate_Tooltip" })}
            pageRef={pageRef}
            isPreviewOpen={isPreviewOpen}
          />
        </Col>
        ...
        <Col span={patchedGetResponsive({ default: 12, tablet: 12, midTablet: 24 })}>
          <CrimeType
            ...
            isRtl={isRtl}
            filters={filters}
            title={intl?.formatMessage({ id: "Crime Rate by Crime Type" })}
            tooltipKey={intl?.formatMessage({ id: "Crime_Rate_by_Crime_Type_Tooltip" })}
            pageRef={pageRef}
            isPreviewOpen={isPreviewOpen}
          />
        </Col>
  ...
  const titleEle = (
    <Row gutter={8} align="middle">
      <Col flex="none" style={{ color: "var(--colorText)" }}>
        <Gavel weight="light" size={getResponsive({ default: "32px", tablet: "24px" })} />
      </Col>
      <Col flex="none">
        ...
        >
          ...
          >
            <Title level={getResponsive({ default: 4, mobile: 5 })}>
              ...
              <Tooltip
                title={intl?.formatMessage({ id: "Crime_Related_Indicators_Tooltip" })}
                placement="bottom"
              >
